...
public class TouchHandler : MonoBehaviour
{
    ...
    void Start()
    {
        mCachedAugmentationScale = AugmentationObject[count].localScale.x;
        mCachedAugmentationRotation = AugmentationObject[count].localEulerAngles;
    }
    ...
    void Update()
    {
        if (Input.touchCount == 2)
        {
            ...
            if (mEnableRotation)
                AugmentationObject[count].localEulerAngles = mCachedAugmentationRotation - new Vector3(0, angleDelta * 3f, 0);
            
            // Optional Pinch Scaling can be enabled via Inspector for this Script Component
            if (mEnableRotation && EnablePinchScaling)
                AugmentationObject[count].localScale = new Vector3(scaleAmountClamped, scaleAmountClamped, scaleAmountClamped);
        }
